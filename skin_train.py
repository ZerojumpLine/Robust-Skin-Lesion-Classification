...
parser.add_argument('--dataset', default='HAM', help='dataset setting')
...
parser.add_argument('--crop', action='store_true', help='crop all images to 224.')
parser.add_argument('--cosine', action='store_true', help='decrease the learning rate with cosine.')
best_acc1 = 0


def main():
    args = parser.parse_args()
    args.store_name = '_'.join([args.dataset, args.loss_type, args.train_rule, args.exp_str])
    if args.crop:
        args.store_name = args.store_name + '_crop'
    if args.cosine:
        args.store_name = args.store_name + '_cosine'
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    num_classes = 7
    model = network(name = 'resnet50', num_classes = 7)
    ...
    if args.crop:
        ResizeTrain = transforms.Resize((224, 224))
        ResizeTest = transforms.Resize((224, 224))
    else:
        ResizeTrain = transforms.RandomResizedCrop(224)
    ...
    # other val_dataset
    # val_dataset = datasets.ImageFolder(root=os.path.join(data_folder, 'BCN'), transform=transform_val)
    # val_dataset = datasets.ImageFolder(root=os.path.join(data_folder, 'D7P'), transform=transform_val)
    # val_dataset = datasets.ImageFolder(root=os.path.join(data_folder, 'MSK'), transform=transform_val)
    # val_dataset = datasets.ImageFolder(root=os.path.join(data_folder, 'PH2'), transform=transform_val)
    ...
    cls_dict = train_dataset.dic
    cls_num_list = [cls_dict[0], cls_dict[1], cls_dict[2], cls_dict[3], cls_dict[4], cls_dict[5], cls_dict[6]]

    # I should give train_loader and val_loader here.
